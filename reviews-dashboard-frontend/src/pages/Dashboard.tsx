...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        const data = res.data.map((r: Review) => ({
          ...r,
  ...
  const handleApprove = (id: string) => {
    ...
    const approved = localStorage.getItem(`approved_${id}`) === "true";
    localStorage.setItem(`approved_${id}`, (!approved).toString());
  ...
  const filteredReviews = reviews
    .filter((r) => {
      const avgRating =
        r.rating ??
        (r.categories.reduce((sum, cat) => sum + cat.rating, 0) /
          r.categories.length ||
          0);
      ...
      const catMatch = filters.category
        ...
        : true;
      const typeMatch = filters.reviewType
        ? r.type === filters.reviewType
        : true;
      return (
  ...
  const properties = Array.from(new Set(reviews.map((r) => r.listing)));
  const propertyStats = properties.map((prop) => {
    ...
    const avgRating =
      totalReviews > 0
        ? Math.round(
            propReviews.reduce((sum, r) => sum + (r.rating ?? 0), 0) /
              totalReviews
          )
        : 0;
    ...
    propReviews.forEach((r) => {
      r.categories.forEach((cat) => {
        categoryAvgs[cat.category] =
          (categoryAvgs[cat.category] || 0) + cat.rating;
      });
    });
    Object.keys(categoryAvgs).forEach((cat) => {
      categoryAvgs[cat] = Math.round(categoryAvgs[cat] / totalReviews);
    });
    const issuesCount = propReviews.filter((r) =>
      ["dirty", "problem", "bad"].some((word) =>
        r.reviewText.toLowerCase().includes(word)
      )
    ).length;
    const issues =
      issuesCount > 1
        ? `${issuesCount} reviews mentionnent des issues`
        : "Aucune issue récurrente";
  ...
  return (
    <div className="bg-[#FCFAF6] text-gray-900 font-sans min-h-screen">
      ...
      {propertyStats.map((stat) => (
        <div
          key={stat.prop}
          className="bg-white  rounded-lg  p-8 my-8 mx-24 rounded-lg shadow-md  mb-2 min-h-screen py-10 px-6  lg:px-16"
        >
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-2xl font-bold text-gray-800 bg-[#284E4C] text-white p-6 shadow-md">
              {stat.prop}
            </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div>
              ...
              <p>
                ...
                <span
                  className={
                    stat.avgRating < 5 ? "text-red-500" : "text-green-500"
                  }
                >
                  {stat.avgRating}/10
                </span>
            ...
            <div>
              ...
              <ul className="list-disc pl-5">
                {Object.entries(stat.categoryAvgs).map(([cat, avg]) => (
                  <li
                    key={cat}
                    className={avg < 5 ? "text-red-500" : "text-gray-600"}
                  >
          ...
          <div className="bg-[#F1F3EE] text-white p-6 shadow-md rounded-md ">
